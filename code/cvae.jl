# this is a copy of the jupytier notebook in this folder for running the code
# on the server
...
data = read(file, "pix3D") close(file)
...
using Flux using Flux: @epochs, mse using Base.Iterators: partition 
using Images using Flux: Conv, MaxPool, Dense, ConvTranspose
...
pool1 = MaxPool((3, 3));
sample2 = pool1(sample1) out_size = size(sample2) 
...
sample6 = layer5(sample5); print("Size of output at layer 5: ", size(sample6))
...
if isfile("cvae_model.bson") && !changed @load "cvae_model.bson" cvae 
end print("Training...") @epochs epochs Flux.train!(loss, params(cvae), 
zip(batches), optimizer) print("done.")
